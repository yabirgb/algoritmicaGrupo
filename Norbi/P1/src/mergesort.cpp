...
int main(int argc, char * argv[])
{
  ...
  const int TAM_GRANDE = 30000;
  const int NUM_VECES = 200;
  ...
  if (n > TAM_GRANDE)
    {
      ...
      cout << n << "  " << (t_despues - t_antes) / CLOCKS_PER_SEC <<endl;
    } else {
      int * U = new int[n];
      assert(U);
      ...
      t_despues = clock();
      cout << n << " " << ((t_despues - t_antes) - 
			 (t_despues_vacio - t_antes_vacio)) / (CLOCKS_PER_SEC * NUM_VECES) <<endl;
